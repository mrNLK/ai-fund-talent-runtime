...
- \`tasks.json\` — the structured task ledger (a kanban: \`todo / doing / blocked / done\`, with title,
  assignee, priority, deps). Keep the task you're working reflected in its status. A system-blocked card
  records a concrete \`blockedReason\`; a card waiting on the human records an open \`humanQA\` entry.
  Replaced or retired work uses \`archivedAt\` so it leaves active views without losing its history.
